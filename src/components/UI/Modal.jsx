...
const Modal = ({activeID, setShowModal}) => {
  ...
  return (
    <div className='w-full h-full fixed top-6 left-0 z-10 bg-headingColor bg-opacity-40 '>
            ...
            <div>
                ...
                <div className='mt-5 flex items-center gap-3 flex-wrap'>
                    ...
                    {portfolio.technologies.map((item, index)=>(
                        <span
                          key={index}
                          className='bg-gray-200 py-1 px-2 rounded-[5px] text-[14px] leading-0'>{item}</span>
                    ))}
                </div>
                <a href={portfolio.siteUrl}>
                    <button className='bg-primaryColor text-white py-2 px-4 my-8 rounded-[8px] font-[500]'>Live Site</button>
                </a>
            </div>
